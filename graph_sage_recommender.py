...
def build_bipartite_graph(users_df, items_df, interactions_df, n_user_features, n_item_features,
    ...
    # Create empty feature tensors for all possible user and item IDs
    ...
    if user_attrs.shape[1] < max_features_dim:
    ...
    if item_attrs.shape[1] < max_features_dim
...
class GraphSageLinkPredictor(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2, aggr_type='mean', decoder_type='mlp', dropout_rate=0.5):
        """
        Initializes a GraphSage-based link prediction model.
        """
        ...
        # Input layer
        self.conv_layers.append(SAGEConv(in_channels, hidden_channels, aggr=aggr_type))
        self.batch_norms.append(torch.nn.BatchNorm1d(hidden_channels))

        # Hidden layers
        for _ in range(num_layers - 1):
            self.conv_layers.append(SAGEConv(hidden_channels, hidden_channels, aggr=aggr_type))
            self.batch_norms.append(torch.nn.BatchNorm1d(hidden_channels))

        # Output layer
        self.final_conv = SAGEConv(hidden_channels, out_channels, aggr=aggr_type)
        if decoder_type == 'mlp':
            ...
            raise ValueError(f"Unsupported decoder type: {decoder_type}. Choose 'dot_product' or 'mlp'.")
    ...
    def forward(self, x, edge_index):
        ...
        for i, conv in enumerate(self.conv_layers):
            x = conv(x, edge_index)
            # Apply BatchNorm before ReLU if there are enough nodes for statistics
            if x.shape[0] > 1:
                x = self.batch_norms[i](x)
            x = F.relu(x)
            x = F.dropout(x, p=self.dropout_rate, training=self.training)
        
        # Final GraphSage layer outputs the node embeddings
    ...
    def decode(self, z, edge_label_index):
        ...
        if self.decoder_type == 'dot_product':
            # Simple dot product between embeddings
...
def train_link_predictor(model, optimizer, data):
    ...
    while sampled_count < num_negative_samples and max_attempts > 0:
        ...
        if (u_idx, i_idx) not in existing_edges:
            train_neg_edge_index_list.append([u_idx, i_idx + num_users]) 
            train_neg_edge_index_list.append([i_idx + num_users, u_idx])
            sampled_count += 1
        max_attempts -= 1

    if not train_neg_edge_index_list and num_negative_samples > 0:
        print("Warning: Could not sample enough negative edges for training. This might affect model training.")
...
class GraphSageRecommender:
    def __init__(self, hidden_channels=64, out_channels=32, num_epochs=100, learning_rate=0.01, 
                 seed=None, spark_session=None, aggr_type='mean', num_layers=2,
                 dropout_rate=0.5, weight_decay=1e-5, patience=10):
        """
        Initializes the GraphSageRecommender with GraphSage and training parameters.
        """
    ...
    def fit(self, log, user_features=None, item_features=None):
        """
        Builds the graph, trains and evaluates the GraphSage-based recommender model.
        """
        print("\n--- Starting GraphSage Recommender System Training ---")
        ...
        # Determine the total number of users and items based on maximum index values
        ...
        data = build_bipartite_graph(
            user_features, item_features, log, # Pass original (Spark) DFs if needed, build_bipartite_graph converts
            n_user_features, n_item_features,
            total_num_users=total_num_users, total_num_items=total_num_items
        )
        print(f"Graph built with {data.num_nodes} nodes and {data.edge_index.shape[1]} edges.")
        print(f"Node feature dimension: {data.x.shape[1]}")

        if data.edge_index.shape[1] < 10:
            return

        print("Splitting edges for training and testing...")
        ...
        print(f"Train pos edges: {data_split.train_pos_edge_index.shape[1] // 2} (undirected)")
        print(f"Val pos edges: {data_split.val_pos_edge_index.shape[1] // 2} (undirected)")
        print(f"Test pos edges: {data_split.test_pos_edge_index.shape[1] // 2} (undirected)")

        # Prepare edge_label_index and edge_label for evaluation sets
        ...
        for epoch in range(1, self.num_epochs + 1):
            loss = train_link_predictor(self.model, optimizer, data)
            val_auc, val_acc = evaluate_link_predictor(self.model, data, val_edge_label_index, val_edge_label, data.num_users)
            ...
            if val_auc > best_val_auc:
                ...
                if patience_counter >= self.patience:
                    print(f"Early stopping at epoch {epoch} due to no improvement in validation AUC for {self.patience} epochs.")
                    break
        
        evaluate_link_predictor(self.model, data, test_edge_label_index, test_edge_label, data.num_users)

    def predict(self, log, k, users, items, user_features, item_features, filter_seen_items=True):
        """
        Generates top-K recommendations for specified users, maximizing expected revenue.
        """
        if self.model is None:
            raise RuntimeError("GraphSageRecommender has not been fitted yet. Call fit() before predict().")
        if not self.fitted_data_info:
             raise RuntimeError("GraphSageRecommender has not been fitted yet. Call fit() before predict().")

        self.model.eval()

        # Convert item_features to Pandas DataFrame
        if not isinstance(item_features, pd.DataFrame):
        ...
        if n_user_features == 0 or n_item_features == 0:
            raise ValueError("User or item features not found in the provided DataFrames for prediction. "
                             "Ensure columns are named 'user_attr_X' and 'item_attr_X'.")
        ...
        if current_data.num_users < self.fitted_data_info['num_users'] or \
           current_data.num_items < self.fitted_data_info['num_items']:
            print("Warning: The graph built for prediction contains fewer unique user/item IDs than the model was trained on. This might affect embedding quality for missing nodes if feature data is incomplete.")

        with torch.no_grad():
            z = self.model(current_data.x, current_data.edge_index) 

        recommendations = []

        # Prepare seen items for filtering (convert log to Pandas if needed)
        if not isinstance(log, pd.DataFrame):
            log_pd = log.toPandas()
        else:
            log_pd = log
        ...
        # Determine the user IDs to predict for (convert to Pandas if needed)
        ...
        # Determine the item IDs to consider as candidates (convert to Pandas if needed)
        ...
        for user_id in user_ids_to_predict:
            candidate_items = list(item_ids_to_consider) 
            ...
            # Create prediction edges: user_id -> candidate_item_id
            ...
            link_probabilities = self.model.decode(z, prediction_edges).sigmoid().cpu().detach().numpy()

            candidate_prices = np.array([item_price_map.get(item_id, 0.0) for item_id in candidate_items]) 
