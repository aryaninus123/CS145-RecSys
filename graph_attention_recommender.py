...
from torch_geometric.nn import SAGEConv, GATConv # Added GATConv
...
def build_bipartite_graph(users_df, items_df, interactions_df, n_user_features, n_item_features,
    ...
    dst_nodes = interactions_df_pd["item_idx"].values + num_users
...
class GATLinkPredictor(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2, heads=4, decoder_type='mlp', dropout_rate=0.5):
        """
        Initializes a GAT-based link prediction model.
        """
        super(GATLinkPredictor, self).__init__()
        self.conv_layers = torch.nn.ModuleList()
        self.batch_norms = torch.nn.ModuleList() # Added Batch Normalization layers
        self.decoder_type = decoder_type
        self.dropout_rate = dropout_rate
        self.heads = heads

        # Input layer
        self.conv_layers.append(GATConv(in_channels, hidden_channels // heads, heads=heads, dropout=dropout_rate, concat=True))
        self.batch_norms.append(torch.nn.BatchNorm1d(hidden_channels)) 

        # Hidden layers
        for _ in range(num_layers - 1):
            self.conv_layers.append(GATConv(hidden_channels, hidden_channels // heads, heads=heads, dropout=dropout_rate, concat=True))
            self.batch_norms.append(torch.nn.BatchNorm1d(hidden_channels))

        # Output layer
        self.final_conv = GATConv(hidden_channels, out_channels, heads=1, dropout=dropout_rate, concat=False)
...
class GraphAttentionRecommender:
    def __init__(self, hidden_channels=64, out_channels=32, num_epochs=100, learning_rate=0.01, 
                 seed=None, spark_session=None, num_layers=2,
                 heads=4, dropout_rate=0.5, weight_decay=1e-5, patience=10):
        """
        Initializes the GraphAttentionRecommender with GAT and training parameters.
        """
    ...
    def fit(self, log, user_features=None, item_features=None):
        """
        Builds the graph, trains and evaluates the GAT-based recommender model.
        """
        ...
        test_auc, test_acc = evaluate_link_predictor(self.model, data, test_edge_label_index, test_edge_label, data.num_users)
    ...
    def predict(self, log, k, users, items, user_features, item_features, filter_seen_items=True):
        ...
        if self.model is None:
            raise RuntimeError("GraphAttentionRecommender has not been fitted yet. Call fit() before predict().")
